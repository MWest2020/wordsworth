...
| 28 | Key-lifecycle stream WORM-exported | `export_key_lifecycle_worm()` verifies the chain, ships the rows to Object Lock incrementally, and checks the stored bytes against the database | **have** · since key-audit-in-postgres (2026-09-23) the stream is a hash-chained, append-only table, so tamper-evidence has two layers like the document chain. **No scheduled caller yet** | `key_audit_export.py`, `key_audit_pg.py` |
