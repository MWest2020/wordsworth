...
- [x] 1.1 Emit rotation events to a separate append-only key-lifecycle audit
      stream (old/new `key_id`, entries re-encrypted, actor; no key material) —
      not `audit.append` on the document chain
- [x] 1.2 Remove `SYSTEM_DOCUMENT_ID` and `_ensure_system_document`; no sentinel
      document rows; `document_id` stays NOT NULL with only real documents

## 2. Tests
- [x] 2.1 A rotation writes a key-lifecycle stream event and leaves the document
      hash-chain unchanged (no new document-audit record, no phantom document)
- [x] 2.2 The document audit table contains no `00000000-…` sentinel document;
