...
- `POST /datasets/pseudonymize` (multipart CSV + JSON profile, or profile by
  name): profile = `{domain, columns: {name: entity_type}, mode:
  per_attribute|per_record, record_key: [columns], format: token|nen7524}`.
  Returns CSV with selected columns replaced; unselected columns untouched.
...
- Optional **PII validation**: unselected columns are sampled through the
  existing detectors; hits are reported as a warning list, never auto-applied
  (the operator decides; matches the "heb je kolommen gemist?" step).
- Every run is one audit record: profile hash, row count, columns, unique
  pseudonyms, domain, `norm_version` — no values. Mappings go into the existing
...
- Code: `datasets.py` (profile model + row transform, ≤ 200 lines),
  ...
  Streaming CSV (stdlib `csv`), no pandas.
