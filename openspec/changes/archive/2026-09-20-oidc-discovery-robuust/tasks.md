...
## 1. Ophalen — run 01
- [x] 1.1 `WORDSWORTH_OIDC_JWKS_URL`: staat die gezet, dan wordt het
  discovery-document overgeslagen en gebruikt de verificatie dat adres.
  De uitgever blijft gecontroleerd tegen `iss`.
- [x] 1.2 Geen netwerkverkeer tijdens het opstarten: ophalen gebeurt bij
  het eerste verzoek dat een token controleert, met cache. Een
  onbereikbare uitgever geeft "geen caller" (fail-closed), geen crash.
- [x] 1.3 De HTTP-aanroep stuurt een User-Agent mee.
- [x] 1.4 Tests: met JWKS-adres wordt discovery niet opgehaald; zonder
