...
# label, pattern, validator (None = geen validatie), context (None = geen
# contextregel). Gedeeld door anonymizer en pseudonymizer, zodat beide precies
# dezelfde PII dekken — en, sinds 2026-09-14, ook precies dezelfde
# uitzonderingen.
#
# Die vierde kolom bestaat om een gemeten reden. De postbus-uitzondering ("een
# postbus houdt zijn postcode") zat in `find_deterministic()` en in
# `redact_postcode()`, maar niet in `substitute()` — en juist `substitute()` is
# wat beide échte pijplijnen aanroepen. Gemeten op het corpus: 304
# postbus-regels, waarvan 214 met een vervángen postcode en precies één met een
# leesbare. De regel stond in de code, in de documentatie en in de tests, en
# draaide in productie nergens.
#
# Erger nog: mijn eigen meting liep via `find_deterministic()` en rapporteerde
# dus keurig dat de uitzondering werkte. Een regel op drie plekken is geen
# regel maar drie regels die toevallig op elkaar lijken.
Context = Callable[[str, int], bool]
DETECTORS: list[
    tuple[str, re.Pattern[str], Callable[[str], bool] | None, Context | None]
] = [
    ("bsn", _BSN_RE, is_valid_bsn, None),
    ("iban", _IBAN_RE, is_valid_iban, None),
    ("email", _EMAIL_RE, None, None),
    ("postcode", _POSTCODE_RE, None, _postcode_context),
]
...
def find_deterministic(text: str) -> list[tuple[str, str, int, int]]:
    ...
    for label, pattern, validate, context in DETECTORS:
        for m in pattern.finditer(text):
            if validate is not None and not validate(m.group(0)):
                continue
            if context is not None and not context(text, m.start()):
                continue
            out.append((label, m.group(0), m.start(), m.end()))
    return out


def substitute(text, pattern, replacer, validate, context=None) -> tuple[str, int]:
    """Replace each validated match via ``replacer(value) -> str``; count them.

    ``context(text, start) -> bool`` mag een match afwijzen op grond van wat
    eromheen staat. Een validator ziet alleen de waarde, en er zijn regels die
    je daarmee niet kunt uitdrukken: een postcode achter "Postbus" is een
    contactadres van een organisatie, dezelfde postcode zonder dat woord wijst
    een huishouden aan. De waarde is identiek; alleen de context verschilt.
    """
    count = 0
    ...
    def repl(match: re.Match[str]) -> str:
        nonlocal count
        value = match.group(0)
        if context is not None and not context(text, match.start()):
            return value
...
def redact_postcode(text: str) -> tuple[str, int]:
    ...
    Eén implementatie van de regel, niet een tweede die op de eerste lijkt.
    """
    return substitute(text, _POSTCODE_RE, lambda v: "[POSTCODE]", None,
                      _postcode_context)
