...
def reach(session, types: set[str]) -> int:
    """In how many documents does EVERY one of these types occur?"""
    wanted = {t.upper() for t in types}
    return sum(1 for present in types_per_document(session).values()
               if wanted <= set(present))
    ...
    def _declared(session, with_reach=None) -> list[dict]:
        ...
        for row in session.execute(select(DeclaredCombination).order_by(
            ...
            if with_reach is not None:
                item["documents"] = reach(with_reach, set(types))
            out.append(item)
        return out
