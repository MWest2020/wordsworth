---
name: reviewer
description: Reviews a change's diff against AGENTS.md invariants + the change itself, nothing else. Read-only; issues a verdict, does not fix.
...
diff against **only** two things: `AGENTS.md` and the change under review
...
5. **AGENTS.md invariants hold** (uv not pip, files ≤200 lines, EUPL/license,
...
- Any diff touching `AGENTS.md`, `.claude/agents/`, or CI config. These are
